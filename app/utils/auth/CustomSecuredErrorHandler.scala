...
class CustomSecuredErrorHandler @Inject() (val messagesApi: MessagesApi) extends SecuredErrorHandler with I18nSupport {
  ...
  override def onNotAuthenticated(implicit request: RequestHeader) = {
    Future.successful(Unauthorized(failedResponse(Message.AuthTokenNotFound)))
  }
  ...
  override def onNotAuthorized(implicit request: RequestHeader) = {
    Future.successful(Forbidden(errorResponse(Message.ForbiddenError)))
  }
}
