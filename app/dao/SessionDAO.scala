...
class SessionDAO @Inject() (val dbConfigProvider: DatabaseConfigProvider, users: UserDAO)(implicit
  ...
  def newSession(user_id: String): Token = {
    ...
    if (exec(db.run(current.exists.result))) {
      exec(
        db.run(
          current
            .map(token => (token.token, token.createdAt, token.expiresAt))
            .update((newToken, timestampNow, anHourFromNow))
        )
      )
      newToken
    } else {
      exec(
        db.run(Sessions += Session(0L, randomID, user_id, newToken, timestampNow, anHourFromNow))
          .map(_ => newToken)
      )
    }
  }
  ...
  def validateSession(token: Token): SessionType = {
    val sessionList = exec(
      db.run(Sessions.filter(session => session.token === token).result).map(res => res)
    )
    token match {
  ...
  private class SessionsTable(tag: Tag) extends Table[models.Session](tag, "sessions") {
    ...
    def user =
      foreignKey("users_fk", user_id, users.Users)(_.user_id, onDelete = ForeignKeyAction.Restrict)
