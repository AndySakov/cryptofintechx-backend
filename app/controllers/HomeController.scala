...
class HomeController @Inject() (
  ...
  /** User creation handler
    ...
    * application receives a `POST` request with a path of `/create/user`.
    */
  def createUser(): Action[AnyContent] = Action.async { implicit request: Request[AnyContent] =>
    {
      createUserForm
        ...
        .fold(
          formWithErrors =>
            Future.successful(BadRequest(UserFormWithErrorsWrites.writes(formWithErrors))),
          userData =>
  ...
  /** User update handler
    *
    * The configuration in the `routes` file means that this method will be called when the
    * application receives a `PUT` request with a path of `/update/user/:part` where part is the
    * detail to update.
    */
  def updateUser(user_id: String): Action[AnyContent] =
    Action.andThen(sessionAuthAction).async { implicit request =>
      updateUserForm
        .bindFromRequest()
        .fold(
          formWithErrors =>
            Future.successful(BadRequest(UserProfileFormWithErrorsWrites.writes(formWithErrors))),
          userProfile =>
  ...
  /** User authentication handler
    *
    * The configuration in the `routes` file means that this method will be called when the
    * application receives a `POST` request with a path of `/auth/user`.
    */
  def authUser(): Action[AnyContent] = Action.async { implicit request: Request[AnyContent] =>
    {
      authUserForm
        .bindFromRequest()
        .fold(
          formWithErrors =>
            Future.successful(BadRequest(AuthFormWithErrorsWrites.writes(formWithErrors))),
          credentials =>
  ...
  /** User deletion handler
    *
    * The configuration in the `routes` file means that this method will be called when the
    * application receives a `DELETE` request with a path of `/delete/user`.
    */
  def deleteUser(user_id: String): Action[AnyContent] =
    Action.andThen(sessionAuthAction).async { implicit request: Request[AnyContent] =>
