...
class UserAccountController @Inject() (
      ...
      .fold(
        ...
        data => {
          ...
          credentialsProvider
                ...
                case Some(user) =>
                  for {
                    ...
                    result <- authenticatorService.embed(
                      token,
                      Ok(
                        successResponse(Message.UserAuthSuccessful, Some(user.copy(password = "")))
                      ),
                    )
                  } yield result
            ...
            .recover {
              case _: ProviderException => Unauthorized(failedResponse(Message.InvalidCredentials))
            }
        },
      )
  }

  def changePassword: Action[AnyContent] = SecuredAction(
  ...
  def updateProfile(userID: String): Action[AnyContent] = SecuredAction(
    WithProvider[AuthType](CredentialsProvider.ID)
  ).async { implicit request: SecuredRequest[JWTEnvironment, AnyContent] =>
    implicit val lang: Lang = supportedLangs.availables.head
    updateProfileForm
  ...
  def deleteUser(userID: String): Action[AnyContent] = SecuredAction(
    WithProvider[AuthType](CredentialsProvider.ID)
  ).async { implicit request: SecuredRequest[JWTEnvironment, AnyContent] =>
    implicit val lang: Lang = supportedLangs.availables.head
    userService.getUserById(userID).flatMap {
