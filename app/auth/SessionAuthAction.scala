...
// handling action
case class SessionAuthRequest[A](request: Request[A], sessionType: SessionType)
    extends WrappedRequest[A](request)

// Our custom action implementation
class SessionAuthAction @Inject() (bodyParser: BodyParsers.Default, sessions: SessionDAO)(implicit
  ec: ExecutionContext
  ...
  override def invokeBlock[A](
    request: Request[A],
    block: SessionAuthRequest[A] => Future[Result]
  ): Future[Result] =
    authSession(request) match {
      case Some(authRequest)
          if authRequest == SessionAuthRequest(authRequest, SessionTypeImpl.VALID) =>
        block(authRequest)
      case Some(authRequest)
          if authRequest == SessionAuthRequest(authRequest, SessionTypeImpl.EXPIRED) =>
        Future.successful(sessionExpired())
      case Some(authRequest)
          if authRequest == SessionAuthRequest(authRequest, SessionTypeImpl.NOT_FOUND) =>
