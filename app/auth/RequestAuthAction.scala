...
// handling action
case class ClientRequest[A](jwt: JwtClaim, token: String, request: Request[A])
    extends WrappedRequest[A](request)

// Our custom action implementation
class RequestAuthAction @Inject() (bodyParser: BodyParsers.Default, authService: AuthService)(
  ...
  override def invokeBlock[A](
    request: Request[A],
    block: ClientRequest[A] => Future[Result]
  ): Future[Result] =
    extractBearerToken(request) map { token =>
      authService.validateJwt(token) match {
        case Success(claim) =>
          block(ClientRequest(claim, token, request)) // token was valid - proceed!
        case Failure(t) =>
          Future.successful(
            Results.Unauthorized(failedResponse(InvalidAuthToken(t.getMessage)))
          ) // token was invalid - return 401
      }
    } getOrElse Future.successful(
      Results.Unauthorized(failedResponse(AuthTokenNotFound))
